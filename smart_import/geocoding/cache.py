...
class GeocodeCache:
    ...
    def get(self, address: str, context: str = "") -> GeocodeResult | None:
        row = self._conn.execute(
            ...
            " FROM geocode_cache WHERE key = ?", (make_key(address, context),)
        ).fetchone()
        if not row:
            self.misses += 1
            return None
    ...
    def put(self, address: str, result: GeocodeResult, context: str = "") -> None:
        self._conn.execute(
            ...
            " (key, normalized_address, lat, lon, status, confidence, precision, source, created_at)"
            " VALUES (?,?,?,?,?,?,?,?,?)",
            (make_key(address, context), result.normalized_address or normalize_text(address),
             ...
             result.precision, result.source, time.time()),
        )
