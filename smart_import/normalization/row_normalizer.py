...
class RowNormalizer:
    ...
    def _validate_coordinates(self, row: NormalizedRow, warn) -> None:
        ...
        if (lat is not None and lng is not None and not lat_ok
            ...
            row.issues.append(
                f"lat={lat} fuera de [-90,90] pero lng={lng} si cabe como latitud: "
                "posible inversion lat/lng (no se corrigio automaticamente)")
        ...
        if lat is not None and not lat_ok:
            row.issues.append(f"latitud fuera de rango [-90,90]: {lat}")
            row.values["lat"] = None
        if lng is not None and not lng_ok:
            row.issues.append(f"longitud fuera de rango [-180,180]: {lng}")
            row.values["lng"] = None

        if (row.values.get("lat") is None) != (row.values.get("lng") is None):
            row.issues.append("coordenada incompleta: hace falta lat Y lng")
    ...
    def _validate_time_window(self, row: NormalizedRow, warn) -> None:
        ...
        if start is None or end is None:
            row.issues.append("ventana horaria incompleta (falta start o end): se descarta")
            ...
            return
        if start > end:
            row.issues.append(f"ventana horaria invertida ({start} > {end}): se descarta")
            row.values["tw_start"] = row.values["tw_end"] = None
    ...
    def _classify(self, row: NormalizedRow) -> None:
        ...
        if has_coords:
            ...
            row.issues.append("fila no localizable: sin coordenadas validas y sin direccion")
